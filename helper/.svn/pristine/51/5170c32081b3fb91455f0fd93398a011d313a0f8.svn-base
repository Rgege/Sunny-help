...
public class ObjectPool <T>{
    ...
    public synchronized void createPool(Object incrementalObjects){
        if(objects !=null ){
            return; // 如果己经创建，则返回
        }
        objects=new Vector();
    ...
    private Object getFreeObject() {
        ...
        if (obj == null) {
            createObjects(new Object());     //如果目前对象池中没有可用的对象，创建一些对象
    ...
    private synchronized void createObjects(Object incrementalObjects){
        if(objects !=null ){
            return; // 如果己经创建，则返回
        }
        objects=new Vector();
